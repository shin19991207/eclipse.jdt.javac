...
public class JavacResolverTaskListener implements TaskListener {
	...
	private void finishedAnalyze(TaskEvent e, JCCompilationUnit u) {
		...
		boolean unnecessaryNLSTagIgnored = this.compilerOptions
					.getSeverityString(CompilerOptions.NonExternalizedString).equals(CompilerOptions.IGNORE);
		...
		List<IProblem> allUnused = getUnusedProblems
				? getUnusedElementProblems(e,
					...
					!noEffectAssignmentIgnored, !unclosedCloseableIgnored, !unusedTypeParameterIgnored, !unnecessaryNLSTagIgnored)
				: new ArrayList<>();
	...
	private List<IProblem> getUnusedElementProblems(TaskEvent e,
			...
			boolean getNoEffectAssignments, boolean getUnclosedCloseables, boolean getUnusedTypeParameters, boolean getUnnecessaryNLSTags) {
		final TypeElement currentTopLevelType = e.getTypeElement();
		...
		if (getUnnecessaryNLSTags) {
			List<CategorizedProblem> unnecessaryNLSTags = scanner.getUnnecessaryNLSTags(unusedProblemFactory);
			if (!unnecessaryNLSTags.isEmpty()) {
				allUnusedProblems.addAll(unnecessaryNLSTags);
			}
		}
